...
export function AddProfileImage() {
  ...
  const backViewDetailsUrl = () => {
    if (backScreenName == "userProfile") {
      navigate('/BrokerProfile?id=' + profileId + "&name=userProfile&pageIndex="+ pageIndex + '&pageStartIndex=' + pageStartIndex,  { replace: true });
    }
    else
      if (backScreenName == "profileList") {
        navigate('/BrokerProfile?id=' + profileIdParam + "&brId=" + brokerId + "&name=profileList&pageIndex=" + pageIndex + '&pageStartIndex=' + pageStartIndex, { replace: true });
      }
      else if(backScreenName == "AddProfileImage"){
         navigate('/BrokerProfile?id=' + profileIdParam + "&name=AddProfileImage")
      }
  ...
  const planates = [
    ...
    'குரு', 'சுக்கிரன்', 'சனி', 'இராகு', 'கேது','மாந்தி'
  ];
  const planatesPdf = [
    'ல', 'சூ', 'சந்', 'செ', 'பு',
    'குரு', 'சுக்', 'சனி', 'ராகு', 'கேது','மா'
  ];



  const options = ['ல', 'சூ', 'சந்', 'செ', 'பு', 'குரு', 'சுக்', 'சனி', 'ராகு', 'கேது','மா'];
  ...
  function constructHoroscope() {
    ...
    if (selectedAmsam.length === 1) {
      ...
      // toast.error(`Please select the remaining Grahas in அம்சம்: ${remainingAmsam.join(', ')}`);
    ...
    dispatch(updateHoroscope(horoscopeDataRef.current))
    setTimeout(function () {
      if (backScreenName == "userProfile") {
        navigate('/BrokerProfile?id=' + profileId + "&brId=" + brokerId + "&name=userProfile", { replace: true })
      }
      else if (backScreenName == "profileList") {
        navigate('/BrokerProfile?id=' + profileId + "&brId=" + brokerId + "&name=profileList&pageIndex=" + pageIndex + '&pageStartIndex=' + pageStartIndex, { replace: true }, { replace: true })
      }
       else if(backScreenName == "AddProfileImage"){
         navigate('/BrokerProfile?id=' + profileIdParam + "&name=AddProfileImage")
      }
    ...
    <div>
      ...
      {(profileUploadImage && profileUploadImage.profileDetails) ? (<div>
        <div><p className="h4">Upload Image</p>
            ...
            <div className="col-8">
              ...
              <label className="form-control-plaintext">:
                {profileUploadImage?.profileDetails?.sex === 'Male' ? (
                  ` UM${profileUploadImage?.profileDetails?.profileID}`
                ) : profileUploadImage?.profileDetails?.sex === 'Female' ? (
                  ` UF${profileUploadImage?.profileDetails?.profileID}`
                ) : (

                  <p>null</p>
                )}
                {/* {profileUploadImage.profileDetails.profileID} */}
              </label>
            </div>
          </div>
          ...
          <div className="row">
            <label style={{ fontWeight: 'bold' }} className="col-4 col-form-label">Mother occu.</label>
            <div className="col-8">
