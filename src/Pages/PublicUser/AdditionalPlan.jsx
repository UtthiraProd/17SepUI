...
import { getPublicUserPlans, upgradePlan, resetUpgradePlan, AdditionalPlans,resetAdditionalPlan, resetGetPUPlan} from "../../Features/Slices/PublicUser/publicUserSlice"
...
export function AdditionalPlan() {
   ...
   const backScreenName = SearchParams.get('name')
    ...
    const { isGetPUPlanLoading, isGetPUPlanSuccess, isPUPlanList, isGetPUPlanError, isUpgradePlanLoading, isUpgradePlanSuccess, isUpgradePlanError,
        ...
        const  handlePay = () => {
         ...
         if (!amount || isNaN(amount)) {
           ...
           verifyRazorpayPayment(response).then(verifyResult => {
                   ...
                   dispatch(AdditionalPlans({profileID, planID, razorpay_order_id, razorpay_payment_id,razorpay_signature}))
                   } else {
        ...
        if (isAdditionalPlanSuccess == true && AdditionalPlanMessage) {
            toast.success(AdditionalPlanMessage)
            dispatch(resetAdditionalPlan())
            navigate('/PublicUserHome')
        }
        if (isAdditionalPlanSuccess == false && AdditionalPlanMessage) {
            toast.error(AdditionalPlanMessage)
            dispatch(resetAdditionalPlan())
        }
        // if (isAdditionalPlanSuccess) {
        //     dispatch(AdditionalPlan({planID:planID,profileId:profileId}))
        // }
    }, [dispatch, isGetPUPlanLoading, isGetPUPlanSuccess, isPUPlanList, isUpgradePlanSuccess, isUpgradePlanError,
        isAdditionalPlanLoading,isAdditionalPlanSuccess,getAdditionalPlan,AdditionalPlanMessage])
    ...
    const onchange = (e) => {
        ...
        const planUpgrade = (clickedPlanID) => {
           ...
           if (clickedPlanID !== planID) {
            ...
            const planUpdateData = { profileID, planID, planPeriod, planDuration, viewCountLimit, downloadCountLimit, viewImageCountLimit,planCost }
            ...
            if (!hasOtherfieldValidation && !hasRequiredfieldValidation) {
                handlePay()
                // dispatch(AdditionalPlans({profileID, planID}))
            }
    ...
    return(<>
            ...
            <div className="container" id="dvuPlan">
                ...
                {isGetPUPlanLoading && !isGetPUPlanSuccess && (
  <>
    {Array.from({ length: isPUPlanList.length || 1 }).map((planList, index) => (
      <div key={index} className="card-skeleton col">
        <div className="skeleton skeleton-image"></div>
        <div className="skeleton skeleton-title"></div>
        <div className="skeleton skeleton-description"></div>
        <div className="skeleton skeleton-description"></div>
